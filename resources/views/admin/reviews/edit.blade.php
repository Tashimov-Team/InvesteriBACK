    ...
    <form action="{{ route('admin.reviews.update', $review) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 gap-6">
            <div>
                ...
                <input value="{{ $review->name }}" name="name" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
            </div>
            ...
            <div>
                ...
                <input value="{{ $review->role }}" name="role" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Текст отзыва</label>
                <textarea name="text" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>{{ $review->text }}</textarea>
            </div>
        </div>
